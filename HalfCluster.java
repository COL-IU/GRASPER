...
public class HalfCluster{
	...
	/*ArrayList<Intger>[] listOfEdgeIntervalIndicies = new ArrayList<Integer>[this.edgeList.size()];
	
	for(int i=0;i<this.edgeList.size();i++){
	    Edge e = this.edgeList.get(i)
	    n += e.getMultiplicity();
	    listOfEdgeIntervalIndicies[i] = new ArrayList<Integer>();
	    for(int j=0; j < e.getMultiplicity(); j++){
		listOfEdgeIntervalIndicies[i].add(new Integer(e.nthInterval(j).getThreadIndex()));
	    }
	    }*/
    ...
    /*if(pairIndicies[0] == 0){
	    delStr = "DELETION\tD1\t" + this.getNumReadsDRSize() + this.pair.getNumReadsDRSize() 
	    ...
	    delStr = "DELETION\tD2\t" + this.pair.getNumReadsDRSize() + this.getNumReadsDRSize()
    ...
    */
    ...
    /*
    public void sortEdgeList(Thread t){
	ArrayList<int[]> listOfIntervalIndicies = new ArrayList<int[]>();
	HashMap<Integer, Edge> index2EdgeHash = new HashMap<Integer, Edge>();


	int[] curIndicies = new int[this.edgeList.size()]; // this is the array to store current indices for intervals
	int[] edgeIndices = new int[this.edgeList.size()]; // this is the corresponding array for curIndicies where cells with same index are pairs.
	int[] nextECs = new int[this.edgeList.size()];

	for(int i=0;i<this.edgeList.size();i++){
	    edgeIndices[i] = i;
	    Edge e = this.edgeList.get(i);
	    int eMul = e.getMultiplicity();
	    listOfIntervalIndicies.put(new int[eMul]);
	    for(int j=0; j < eMul; j++){
		int curThreadIndex = e.nthInterval(j).getThreadIndex();
		if(j==0)
		    curIndicies[i] = curThreadIndex;
		listOfIntervalIndicies.get(i)[j] = curThreadIndex;
		index2EdgeHash.put(new Integer(curThreadIndex),e);
	    }
	...
	while(){
	    sort(curIndicies, edgeIndices);
	    for(int i=1;i<curIndicies.length;i++){
		EdgeCluster eClu0 = this.edgeClusterHash.get(index2EdgeHash.get(curIndicies[0]));
		EdgeCluster eClui = this.edgeClusterHash.get(index2EdgeHash.get(curIndicies[i]));
		
		if(!eClu0.withinDm(eclui, t.getNthInterval(curIndicies[0]).getEC(), t.getNthInterval(curIndicies[i]).getEC(), Constants.DM)){
		    feedNextIndicies(curIndicies, edgeIndicies, nextECs, listOfIntervalIndicies);
		    break;
		}
	    }
    ...
    private void feedNextIndicies(int[] curIndicies, int[] edgeIndicies, int[] nextECs, ArrayList<int[]> listOfIntervalIndicies){
	int[] tmp = listOfIntervalIndicies.get(edgeIndicies[0]);
	if(tmp.length > nextECs[0]){
	    curIndicies[0] = tmp[nextECs[0]];
	    nextECs[0]++;
	}else{
	    curIndicies[0] = -1;
	    nextECs[0]++;
    ...
    */
    ...
    //SHOULD NOT BE USED
    //NOT USING THIS AT THE MOMOMENT
    /*
    private boolean hasSameOrientationAs(HalfCluster other){
	if( this.positionRead.isFwd() == other.getPositionRead().isFwd()
	    && this.pair.getPositionRead().isFwd() == other.getPair().getPositionRead().isFwd() )
	    return true;
	return false;
	}*/
    ...
    /*
    public boolean hasSameOrientationAs(HalfCluster other, int thisEC, int otherEC){
	if(!hasSameLeadingOrientationAs(other, thisEC, otherEC))
	    return false;
	this.getPair
	}*/
